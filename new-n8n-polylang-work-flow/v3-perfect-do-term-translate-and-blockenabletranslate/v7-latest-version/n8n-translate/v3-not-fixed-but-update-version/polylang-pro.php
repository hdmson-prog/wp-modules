 ...
 * Changelog:
 *   v1.4.4 — 2026-03-18
 *     - FIX: plt_get_untranslated_posts() hardcoded querying pll_default_language() as
 *       the source language. On sites where the writing language differs from Polylang's
 *       default (e.g. Polylang default=en but content written in zh), this queried the
 *       wrong language and returned posts:[] every time.
 *       Added a new optional 'source_lang' query param. When passed, the endpoint queries
 *       posts in that language instead of the Polylang default. Falls back to the Polylang
 *       default when not passed, preserving backward compatibility.
 *       Also fixed the skip-self check to use $query_lang instead of $default_lang, so
 *       the source language is correctly excluded from missing_langs regardless of which
 *       language Polylang considers the default.
 *
 *   v1.4.3 — 2026-03-18
...
add_action('rest_api_init', function () {
    ...
    register_rest_route($ns, '/untranslated-posts', [
        ...
        'args' => [
            ...
            'source_lang' => ['default' => '', 'sanitize_callback' => 'sanitize_text_field'], // ← NEW: override which lang to query posts from
            'target_lang' => ['default' => '', 'sanitize_callback' => 'sanitize_text_field'],
...
function plt_get_untranslated_posts(WP_REST_Request $request)
{
    ...
    $source_lang = $request->get_param('source_lang'); // ← the language your posts are written in
    ...
    // Use the explicitly passed source_lang if provided, otherwise fall back to
    // Polylang's default language. This allows sites where the writing language
    // differs from the Polylang default (e.g. default=en but posts written in zh).
    $query_lang = !empty($source_lang) ? $source_lang : $default_lang;
    ...
    foreach ($query->posts as $post) {
        ...
        foreach ($check_langs as $lang) {
            // Skip the source language itself — we never translate a post into its own language
            if ($lang === $query_lang)
                continue;
