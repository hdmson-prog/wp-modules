 ...
 *              post thumbnails, language linking via Polylang, and Gutenberg block content.
 * Version: 1.1.0
...
add_action('rest_api_init', function () {
    ...
    // 3. Get full post payload (content + meta + thumbnail + blocks) for a single post
...
function plt_get_post_payload(WP_REST_Request $request)
{
    ...
    // ── Detect if this post uses Gutenberg blocks ──
    ...
    $payload = [
        ...
        'translatable' => [
            'title' => $post->post_title,
            'content' => $post->post_content, // kept for non-block fallback
            'excerpt' => $post->post_excerpt,
            'slug' => $post->post_name,
        ],
        // ── NEW: Parsed block segments for Gutenberg pages ──
        'blocks' => $has_blocks ? plt_extract_block_texts($post->post_content) : [],
...
// ─────────────────────────────────────────────
// SECTION 5b — GUTENBERG BLOCK EXTRACTION
// ─────────────────────────────────────────────

/**
 * Parse Gutenberg block content and extract all translatable text segments.
 *
 * Returns a flat array of segments, each with:
 *   index       — sequential position (used to reassemble)
 *   block_type  — e.g. "core/paragraph", "core/heading", "acf/hero"
 *   path        — dot-notation path within the block for nested attrs
 *   text        — the translatable string
 *   is_html     — whether the text is innerHTML (true) or an attribute (false)
 *
 * The original post_content is stored so it can be reconstructed by
 * replacing each segment back in the correct position after translation.
 */
...
/**
 * Recursively walk parsed blocks and collect text segments.
 */
function plt_walk_blocks(array $blocks, array &$segments, int &$index, array $parent_path): void
{
    // Block types whose innerHTML is the primary translatable content
    $html_content_blocks = [
    ...
    // Block attribute keys that contain translatable plain text (not HTML)
    $translatable_attr_keys = [
        'content', 'caption', 'alt', 'label', 'title', 'text',
        'placeholder', 'buttonText', 'value', 'citation',
    ...
    foreach ($blocks as $block_index => $block) {
        ...
        $block_path = array_merge($parent_path, [$block_index]);
        $path_str = implode('.', $block_path);

        // ── 1. innerHTML for known content blocks ──
        if (in_array($block_type, $html_content_blocks, true)) {
        ...
        // ── 2. Named attributes for ALL block types ──
        foreach ($block['attrs'] ?? [] as $attr_key => $attr_val) {
            if (!in_array($attr_key, $translatable_attr_keys, true))
                continue;
            if (!is_string($attr_val) || trim($attr_val) === '')
                continue;
            ...
            $segments[] = [
                ...
                'block_type' => $block_type,
                'path' => $path_str,
                'sub_path' => 'attrs.' . $attr_key,
                'text' => $attr_val,
                'is_html' => (bool)preg_match('/<[^>]+>/', $attr_val),
            ];
        }

        // ── 3. innerBlocks — recurse ──
        if (!empty($block['innerBlocks'])) {
            plt_walk_blocks($block['innerBlocks'], $segments, $index, $block_path);
        }
    }
}

/**
 * Reassemble Gutenberg post_content after translation.
 *
 * $translated_segments: array of [ index => translated_text ]
 * $original_content: the raw post_content string
 * $original_segments: the segments array from plt_extract_block_texts()
 *
 * Strategy: rebuild by replacing each segment's text inside the serialized
 * block string. We use a deterministic approach — parse → modify → serialize.
 */
function plt_rebuild_block_content(
    string $original_content,
    array $original_segments,
    array $translated_segments
): string {
    if (!function_exists('parse_blocks') || !function_exists('serialize_blocks')) {
        return $original_content;
    }

    if (empty($original_segments) || empty($translated_segments)) {
        return $original_content;
    }

    // Build a map: index => translated_text
    $translation_map = [];
    foreach ($translated_segments as $seg) {
        $translation_map[(int)$seg['index']] = $seg['translated_text'];
    }

    $blocks = parse_blocks($original_content);
    plt_apply_translations_to_blocks($blocks, $original_segments, $translation_map);

    return serialize_blocks($blocks);
}

/**
 * Apply translated text back into the parsed block tree.
 */
function plt_apply_translations_to_blocks(
    array &$blocks,
    array $segments,
    array $translation_map
): void {
    // Build segment lookup by path + sub_path for fast access
    $seg_map = [];
    foreach ($segments as $seg) {
        $key = $seg['path'] . '|' . $seg['sub_path'];
        $seg_map[$key] = $seg;
    }

    plt_recursive_apply($blocks, $seg_map, $translation_map, []);
}

function plt_recursive_apply(
    array &$blocks,
    array $seg_map,
    array $translation_map,
    array $current_path
    ...
    foreach ($blocks as $bi => &$block) {
        $path = array_merge($current_path, [$bi]);
        $path_str = implode('.', $path);

        // Apply innerHTML replacement
        $html_key = $path_str . '|innerHTML';
        if (isset($seg_map[$html_key])) {
            $seg_index = $seg_map[$html_key]['index'];
            if (isset($translation_map[$seg_index])) {
                $block['innerHTML'] = $translation_map[$seg_index];
                // Also update innerContent (WP stores duplicate for serialization)
                if (!empty($block['innerContent'])) {
                    foreach ($block['innerContent'] as &$ic) {
                        if (is_string($ic) && trim($ic) !== '') {
                            $ic = $translation_map[$seg_index];
                            break;
                        }
                    }
                    unset($ic);
        ...
        foreach ($block['attrs'] ?? [] as $attr_key => $attr_val) {
            $attr_path_key = $path_str . '|attrs.' . $attr_key;
            if (isset($seg_map[$attr_path_key])) {
                $seg_index = $seg_map[$attr_path_key]['index'];
                if (isset($translation_map[$seg_index])) {
                    $block['attrs'][$attr_key] = $translation_map[$seg_index];
                }
            }
        }
        ...
        if (!empty($block['innerBlocks'])) {
            plt_recursive_apply($block['innerBlocks'], $seg_map, $translation_map, $path);
        }
    }
    unset($block);
}
...
// ─────────────────────────────────────────────
// SECTION 5c — META HELPERS (unchanged)
// ─────────────────────────────────────────────
...
function plt_save_translation(WP_REST_Request $request)
{
    ...
    // ── NEW: Block translation payload ──
    // { original_content, segments: [{ index, translated_text }] }
    $blocks_payload = $body['blocks'] ?? null;
    ...
    // Priority: translated blocks > translated.content > source content
    $final_content = $source_post->post_content;

    if (!empty($blocks_payload) && !empty($blocks_payload['original_content'])) {
        // Reconstruct Gutenberg content from translated block segments
        $final_content = plt_rebuild_block_content(
            $blocks_payload['original_content'],
            $blocks_payload['segments'] ?? [],
            $blocks_payload['translated_segments'] ?? []
        );
    } elseif (!empty($translated['content'])) {
        // Fallback: plain content replacement (classic editor)
    ...
    return rest_ensure_response([
        ...
        'blocks_translated' => !empty($blocks_payload['translated_segments']),
